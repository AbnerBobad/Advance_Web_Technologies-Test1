...
**Not Yet Implemented:** Measurements (acknowledgment, queue wait, processing, job duration, polling count, detection delay), the five-image burst experiment, and the final Week 4 integration checklist.
 
---
 
## Progress Summary
 
The polling process begins once a POST request is made and the status of `202 Accepted` is received along with the `status_url` parameter (`beginObservingJob` function in `frontend/app.js`). File select does not initiate any polling activity. The `app.js` runs a loop that sends a `GET /v1/jobs/{id}` request every second as long as the current status is either `queued` or `processing`, and stops once the status is `completed` or `failed`. The response will be fetched in the `fetchJobStatus` function from `frontend/modules/data-service.js`. The function handles both aborts and a 5-second delay per request using `AbortSignal.any`.

UI displays the four steps of the official process (accepted upload, original saved, variants generation, completed) based on the job status and timestamps (`buildTimeline` in `frontend/render.js`), changes the status icon and timestamps for each response, and keeps the result pane with status "Images are being generated" until it reaches `completed`. Result cards with image names, actual sizes, and download/view icons are displayed only at completion; if a job is `failed`, only the safe error is displayed.

Retrieval errors conform to the specified decision rules: the job and its most recent state are retained, automatic polling is terminated, and the card presents the option "Try again" ("handleRetrievalError"/"resumePolling"). "Try again" resubmits the current status_url without resubmitting the image, hence allowing it to detect that the job has been completed while the observation process was temporarily paused. Accepting a new job will cancel the observation of the previous job, and a beforeunload event handler will cancel any active polling process if the page is closed ("stopPolling," triggered by "AbortController"). Smoke suite's polling cycle already monitors the transition from "queued" to "processing" to "completed." go build, go vet, and go test all succeed.
